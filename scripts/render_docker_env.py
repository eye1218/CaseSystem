...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Require GitLab CI/CD variables for production secrets and fixed configuration.",
    )
...
def choose_secret(
    ...
    if override:
        return override
    state_value = state.get(name, "")
    if state_value and state_value != template.get(name, ""):
        return state_value
...
def validate_strict_inputs() -> None:
    missing = []
    ...
    if missing:
        raise SystemExit(
            "Missing required GitLab CI/CD variables in strict mode: " + ", ".join(missing)
        )
...
def write_state_file(path: Optional[Path], values: dict[str, str]) -> None:
    if path is None:
        return

    state_values = {
        "POSTGRES_PASSWORD": values["POSTGRES_PASSWORD"],
        "CASESYSTEM_JWT_SECRET_KEY": values["CASESYSTEM_JWT_SECRET_KEY"],
    }
    if values["CASESYSTEM_SMTP_PASSWORD"]:
        state_values["CASESYSTEM_SMTP_PASSWORD"] = values["CASESYSTEM_SMTP_PASSWORD"]
    ...
    for key in ("POSTGRES_PASSWORD", "CASESYSTEM_JWT_SECRET_KEY", "CASESYSTEM_SMTP_PASSWORD"):
        if key in state_values:
            lines.append(f"{key}={render_env_value(state_values[key])}")
    tmp_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    tmp_path.replace(path)
    try:
        path.chmod(0o600)
...
def main() -> None:
    ...
    postgres_password = choose_secret("POSTGRES_PASSWORD", existing, state, template, 16)
    jwt_secret_key = choose_secret("CASESYSTEM_JWT_SECRET_KEY", existing, state, template, 32)

    smtp_active = any([smtp_host, smtp_username, smtp_from_email])
    smtp_password = env_override("CASESYSTEM_SMTP_PASSWORD")
    if smtp_password is None and "CASESYSTEM_SMTP_PASSWORD" in state:
        smtp_password = state["CASESYSTEM_SMTP_PASSWORD"]
    if smtp_password is None and "CASESYSTEM_SMTP_PASSWORD" in existing:
        smtp_password = existing["CASESYSTEM_SMTP_PASSWORD"]
    if smtp_password == template.get("CASESYSTEM_SMTP_PASSWORD", ""):
        smtp_password = ""
    ...
    write_state_file(args.state, values)
