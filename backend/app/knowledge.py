...
from app.models import KnowledgeArticle, KnowledgeArticleLike
...
def _validate_category(category_id: str) -> None:
    if category_id not in TICKET_CATEGORY_NAMES:
...
def _summary_payload(article: KnowledgeArticle, *, likes_count: int) -> dict:
    return {
        "id": article.id,
        "title": article.title,
        "category_id": article.category_id,
        "category_name": TICKET_CATEGORY_NAMES[article.category_id],
...
def _detail_payload(article: KnowledgeArticle, actor: ActorContext, *, likes_count: int, viewer_has_liked: bool) -> dict:
...
def _likes_count(db: Session, article_id: str) -> int:
    return db.scalar(
        select(func.count(KnowledgeArticleLike.id)).where(KnowledgeArticleLike.article_id == article_id)
    ) or 0
...
def list_articles(db: Session, actor: ActorContext, *, category_id: str | None = None) -> dict:
...
def get_article_detail(db: Session, actor: ActorContext, article_id: str) -> dict:
    _assert_internal_actor(actor)
...
def create_article(db: Session, actor: ActorContext, *, title: str, category_id: str, content_markdown: str) -> dict:
    _assert_internal_actor(actor)
    _validate_category(category_id)
    now = utcnow()
    article = KnowledgeArticle(
...
def like_article(db: Session, actor: ActorContext, article_id: str) -> dict:
    _assert_internal_actor(actor)
    article = _fetch_article(db, article_id)
    existing = db.scalar(
...
def unlike_article(db: Session, actor: ActorContext, article_id: str) -> dict:
    _assert_internal_actor(actor)
    article = _fetch_article(db, article_id)
    db.execute(
...
def pin_article(db: Session, actor: ActorContext, article_id: str, *, pinned: bool) -> dict:
    _assert_internal_actor(actor)
    _assert_admin(actor)
    article = _fetch_article(db, article_id)
...
def list_related_articles_for_ticket_detail(db: Session, actor: ActorContext, *, category_id: str) -> list[dict]:
    if actor.active_role == RoleCode.CUSTOMER.value:
        return []
    rows: Iterable[tuple[KnowledgeArticle, int]] = db.execute(_article_statement(category_id=category_id)).all()
