...
from casesystem.enums import CounterType, RefreshTokenStatus, RoleCode, SecurityEventType, SessionStatus, UserStatus
from casesystem.models import AuthLoginCounter, AuthSecurityEvent, AuthSession, CsrfToken, RefreshToken, User, UserRole, UserRoleSession
from casesystem.policies import ObjectScope, has_object_access, has_permission
from casesystem.schemas import AuthResponse, AuthenticatedUser
from casesystem.security import (
    create_access_token,
    decode_access_token,
    generate_csrf_token,
...
class AuthService:
    ...
    def refresh(self, *, request: Request, response: Response) -> AuthResponse:
        refresh_token = request.cookies.get("__Secure-refresh_token")
    ...
    def _clear_auth_cookies(self, response: Response) -> None:
        response.delete_cookie("__Host-access_token", path="/", domain=self.settings.cookie_domain)
        response.delete_cookie("__Secure-refresh_token", path="/auth/refresh", domain=self.settings.cookie_domain)
